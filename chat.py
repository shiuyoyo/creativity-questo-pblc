...
class LLM:
    def __init__(self):
        ...
        try:
            LLM_Classifier = ChatOpenAI(model="gpt-4o-mini", api_key=api_key)
            self.LLM_Classifier = LLM_Classifier.with_structured_output(CLSOutput)

            LLM_SCAMPER = ChatOpenAI(model="gpt-4o-mini", api_key=api_key)
            self.LLM_SCAMPER = LLM_SCAMPER.with_structured_output(SCAMPEROutput)

            LLM_Guidance = ChatOpenAI(model="gpt-4o-mini", api_key=api_key)
